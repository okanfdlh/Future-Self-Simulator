...
export async function POST(req) {
  ...
  if (!validation.ok)
    return NextResponse.json(
      { error: "invalid_state", details: validation.errors },
      { status: 400 }
    )
  ...
  if (!generateVideo) return NextResponse.json({ runs: runsWithFallback })

  try {
    const enriched = []
    for (const run of runsWithFallback) {
    ...
    return NextResponse.json(
      { error: e?.message || "pixverse_error", runs: runsWithFallback },
      { status: 500 }
    )
  }
}
